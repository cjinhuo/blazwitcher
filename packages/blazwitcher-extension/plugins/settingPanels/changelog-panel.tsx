	...
	const getIframeHtml = () => {
		const html = (releases as Release[])
			.map((release, index) => {
				...
				<div style="margin-bottom:32px;padding:20px;background-color:var(--semi-color-fill-0,#fff);border-radius:8px;border-left:3px solid var(--semi-color-primary-light-default,#e0eaff);transition:all 0.2s ease;">
					<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:8px;">
						<div style="display:flex;align-items:center;margin-bottom:16px;gap:12px;">
							<span style="font-size:20px;font-weight:700;color:var(--semi-color-primary,#0052cc);margin-right:10px;cursor:pointer;transition:color 0.2s;" title="查看发布：${release.tag_name}" onclick="window.open('${release.html_url}','_blank')">v${release.tag_name}</span>
				...
				${index < (releases as Release[]).length - 1 ? '<div style="height:1px;background-color:var(--semi-color-border,#e0e0e0);margin:24px 0;opacity:0.7;"></div>' : ''}
			`
			})
			.join('')
		return `
			...
			<head>
				...
				<style>
					...
					h1, h2, h3, h4, h5, h6 { color: var(--semi-color-primary, #1677ff); margin-top: 1.5em; margin-bottom: 0.5em; }
